...
class NowWeatherView : View {
  ...
  private val cloudPaint2: Paint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
    color = ContextCompat.getColor(context, R.color.cloud_color2)
    style = Paint.Style.FILL
  }
  ...
  private val waveLength2 = 900


  // 偏移量
  private var mOffset = 0

  private val cloudAnim = ValueAnimator.ofInt(0, waveLength)
    .apply {
  ...
  private fun drawCloudy(canvas: Canvas) {
    ...
    cloudPath.reset();
    cloudPath.moveTo(-waveLength + mOffset * 1f, centerY * 1f);//移到屏幕外最左边

    for (i in 0..waveCount) {
      cloudPath.quadTo(
        (-waveLength * 3 / 4) +
            (i * waveLength * 1f) + mOffset, centerY + 60f, (-waveLength / 2f) +
            (i * waveLength) + mOffset, centerY * 1f
      );
      cloudPath.quadTo(
        (-waveLength / 4) +
            (i * waveLength * 1f) + mOffset, centerY - 60f,
        i * waveLength + mOffset * 1f, centerY * 1f
      );
    }
    //填充矩形
    cloudPath.lineTo(screenWidth * 1f, 0f);
    cloudPath.lineTo(0f, 0f);
    cloudPath.close();
